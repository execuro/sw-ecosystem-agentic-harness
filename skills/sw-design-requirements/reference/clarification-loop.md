...
**Recommended options must be advised — mandatory.** Never mark `(recommended)` on your own judgement; an agent recommends, this skill only records. In editor mode that happens in the run that raises the question — see *Editor mode*, *Advice is part of raising a question*.
...
- Disagree → keep `sw-product-manager`'s mark, both notes. Exactly two unadvised states are legal, both written out: `[pm] no recommendation — business preference` (the agent advised and declined) and `[user] raised on the page — advice not requested` (the user's own question, editor mode). A question carrying neither a `(recommended)` option nor one of those notes is a defect, not a state.
