...
// no `which`. It is allowed to be wrong in the harmless direction: a host that
// is installed but has no marker directory simply is not written to, and
// `--host` forces it.
...
export const HOSTS = ['claude-code', 'codex', 'copilot', 'cursor'];

/** Where each host reads its skills from — the base every skill-copy and
 * companion skill-copy action is rooted at. */
...
export async function detectAll(ctx) {
  return Promise.all(HOSTS.map((id) => Promise.resolve(detect(id, ctx))));
}
...
/** Build the full action list for the selected hosts. */
export function buildPlan(ctx) {
  const actions = [];
  for (const id of ctx.hosts) actions.push(...planFor(id, ctx));
...
function planClaude(ctx) {
  ...
  const out = [
    ...
    ...(ctx.companions === false ? [] : companionActions(host, SKILL_DIR[host])),
...
function planCursor(ctx) {
  ...
  // `ctx.hosts` is narrowed to one host while previewing, so the question
  // "is Claude Code also being installed?" must be asked of the user's actual
  // selection, which `selectedHosts` carries unchanged.
  const selected = ctx.selectedHosts ?? ctx.hosts;
  ...
  if (!claudeTrees) {
    ...
    if (ctx.companions !== false) out.push(...companionActions(host, SKILL_DIR[host]));
