...
/**
 * Collapse CRLF (and stray CR) to LF. A fence's identity is what sits between
 * its markers, not the line-ending style the target file happens to use —
 * `writeAtomic` writes back whatever EOL the file already had, so hashing raw
 * bytes would make the same content look edited on a CRLF checkout.
 */
