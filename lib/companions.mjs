// The two optional companion packages, probed rather than installed by us.
//
...
// copying it ourselves through `writeAtomic` keeps every companion skill on
// the same adopt/conflict/drift/update ladder as everything else this CLI
// writes, and makes `uninstall` exact even after the user has removed the
// companion package — the common case, since it is optional.
//
...
export function probeArgs(companion, platform) {
...
function doProbe(companion, platform) {
  const spec = `${companion.pkg}@${companion.version}`;
  const base = { id: companion.id, pkg: companion.pkg, version: companion.version, spec, label: companion.label };
  const wrapped = wrapCommand(
    { command: 'npx', args: ['--no-install', spec, 'install-skill', '--print'] },
    platform,
  );
...
/** Probe one companion. Never throws. */
export function probe(companion, { platform = process.platform } = {}) {
  const key = `${companion.pkg}@${companion.version}`;
  if (cache.has(key)) return cache.get(key);
  const result = doProbe(companion, platform);
  cache.set(key, result);
  return result;
}

export function probeAll(opts) {
  return COMPANIONS.map((c) => probe(c, opts));
}
