...
import { COMPANIONS, probe } from './companions.mjs';
...
function previewSkill(action, ctx) {
  const companion = COMPANIONS.find((c) => c.id === action.companion);
  const probed = probe(companion, { platform: ctx.platform });
...
export function execute(action, ctx) {
  ...
  switch (action.kind) {
    ...
    case 'skill-copy': {
      ...
      // `lock.entries`, `revert` and `rebuild` need no companion-specific
