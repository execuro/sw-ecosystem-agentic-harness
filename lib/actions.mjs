...
import {
  canonical, deletePointer, fenceInner, findFence, getPointer, mergeArrayUnique, normalizeEol,
  parseJsonLoose, removeArrayValues, removeFence, serializeJson, setPointer, upsertFence,
} from './merge.mjs';
import { PKG_ROOT } from './content.mjs';
import { isDeclined } from './lock.mjs';
import { readFileSync } from 'node:fs';
import { EXTRA_COMPONENTS, probe } from './extra-components.mjs';
...
export function preview(action, ctx) {
  switch (action.kind) {
    ...
    case 'skill-copy': return previewSkill(action, ctx);
...
function previewFile(action, ctx) {
  ...
  return previewBytes(action, ctx, shipped);
}

/** The adopt/conflict/drift/update ladder, shared by `file-copy` and, with a
 * probed body instead of a shipped one, `skill-copy`. */
function previewBytes(action, ctx, bytes) {
  const hash = sha256(bytes);
  const target = abs(ctx.root, action.target);
  const current = readMaybe(target);
  const locked = ctx.lock?.files?.[action.target] ?? null;
  const base = { ...action, hash, bytes: bytes.length };
...
/** Strip the probed body (a Buffer) before it lands anywhere near the
 * action, which `commands.mjs` serialises straight to stdout. */
function stripProbeBody({ body, ...rest }) {
  return rest;
}

function previewSkill(action, ctx) {
  const component = EXTRA_COMPONENTS.find((c) => c.id === action.component);
  const probed = probe(component, { platform: ctx.platform });
  const probeSummary = stripProbeBody(probed);

  if (!probed.available) {
    const remedy = probed.state === 'absent'
      ? `npm i -D ${probed.spec}`
      : 'check that npx works — probing needs network-free npx --no-install';
    return {
      ...action, state: 'skipped', available: false,
      probe: probeSummary, reason: probed.reason, remedy,
    };
  }

  return {
    ...previewBytes(action, ctx, probed.body),
    available: true, probe: probeSummary, body: probed.body,
  };
}
...
function previewFence(action, ctx) {
  ...
  // can be compared with what `findFence` reads back. Normalised to LF first —
  // a fence's identity does not depend on the target file's line-ending style.
  const hash = sha256(normalizeEol(fenceInner(action.body, action.syntax)));
  ...
  const priorHash = sha256(normalizeEol(existing.inner));
...
export function execute(action, ctx) {
  ...
  switch (action.kind) {
    ...
    case 'skill-copy': {
      // Collapses to a `file-copy` lock entry on purpose: `lock.record`,
      // `lock.entries`, `revert` and `rebuild` need no extra-component-specific
      // case, and `uninstall` needs no probe. `source` is provenance only —
      // `revert` never reads it.
      ensureDir(dirname(target));
      writeAtomic(target, action.body);
      const dirs = ancestorDirs(ctx.root, action.target);
      return { kind: 'file-copy', host: action.host, target: action.target,
        source: `npm:${action.package}#install-skill --print`,
        hash: action.hash, bytes: action.bytes, created_file: action.created_file !== false, dirs };
    }
    case 'json-merge': {
...
export function revert(entry, ctx) {
  ...
  switch (entry.kind) {
    ...
    case 'text-fence': {
      ...
      if (sha256(normalizeEol(fence.inner)) !== entry.hash) {
