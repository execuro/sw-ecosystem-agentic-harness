...
This CLI installs the Shopware agentic harness — 11 \`sw-*\` skills and 7
...
1. \`${RUN} status\`
   Read-only. Reports which hosts are present, what is already installed, and
   anything that has drifted since it was installed. Always safe.
...
3. \`${RUN} apply --yes\`
   Performs the changes. It refuses without \`--yes\`; there is no prompt,
   because Codex's question tool does not block outside Plan mode and CI has no
   terminal.
...
## Flags
  ...
  --no-companions                           skip the optional companion-package skills
...
(\`npx --no-install\`) and never fetch them. A \`skipped\` action means the
package is not on this machine yet: install it with
\`npm i -D <package>@<version>\` (the exact command is in the action's
\`remedy\`, and in \`status\`'s \`companions\` array), then re-run
\`${RUN} apply --yes\`.
...
export async function main() {
  return out.ok('guide', {
    ...
    help: [`${RUN} status`, `${RUN} plan`, `${RUN} apply --yes`],
    guide: GUIDE,
  });
}
