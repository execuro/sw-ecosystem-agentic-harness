...
1. \`${RUN} install\`
   ...
   the verb is the consent — but it does still need to know **which** host(s)
   to write into, resolved in order: \`--host\`, if given; else the hosts a
   lock file already records, if this is a re-run; else, with a real
   terminal attached, an interactive picker; else it exits 2 with the four
   \`${RUN} install --host <h>\` commands as \`help\` rather than guess or
   hang — Codex's question tool does not block outside Plan mode, and CI has
   no terminal, so a skill invoking this must always pass \`--host\`.
...
4. \`${RUN} apply --yes\`
   ...
   host(s) to touch the same way \`install\` does (see step 1).
...
## Flags
  ...
  --host claude-code|codex|copilot|cursor   select one host, repeatable —
                                             see the host-resolution order in
                                             step 1 above; a skill invoking
                                             \`install\`/\`apply\` must always
                                             pass it, since it has no terminal
...
- **Exit codes:** 0 success, 2 usage error (your invocation was wrong,
  including \`install\`/\`apply\` run with no \`--host\`, no existing lock and
