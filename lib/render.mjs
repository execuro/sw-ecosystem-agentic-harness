...
function renderApply(body, opts, t) {
  ...
  const migrated = body.migrated ?? [];

  if (changed === 0 && problems.length === 0 && migrated.length === 0) {
  ...
  const lines = [];
  if (changed) lines.push(`${t.green('✓')} ${verb} the Shopware agentic harness.`);
  else if (problems.length) lines.push(`${t.yellow('!')} Nothing was written — see below.`);
  // Only tidy-up ran: the install itself was already current.
  else lines.push(`${t.green('✓')} Shopware agentic harness: already up to date.`);
  if (groups.length) {
    lines.push('');
  ...
  if (migrated.length) {
    lines.push('');
    for (const m of migrated) lines.push(`  ${t.dim('·')} ${m.target}: ${m.detail}`);
  }
  if (problems.length) {
    lines.push('');
