...
import { isDir, walk } from './fsx.mjs';
...
/**
 * Where the lock lives, relative to the install root.
 *
 * A Shopware project ignores the whole of `var/` through the `shopware/core`
 * Flex recipe (`/var/*` with a single `!/var/.htaccess` negation), and `var/`
 * is already where that project keeps machine-local state — `var/cache`,
 * `var/log`. Writing the lock there keeps the repository root clean and means
 * the installer needs no `.gitignore` line at all.
 *
 * A root with no `var/` — a user-scope install under $HOME, or a checkout that
 * is not a Shopware project — keeps the original dot-directory, which is
 * gitignored through the managed block as before. Nothing here looks at the
 * platform or at any path outside the install root.
 */
export const LOCK_DIR = '.sw-ai-sdk';
export const LOCK_FILE = `${LOCK_DIR}/harness.lock.json`;
export const VAR_LOCK_DIR = 'var/sw-ai-sdk';
export const VAR_LOCK_FILE = `${VAR_LOCK_DIR}/harness.lock.json`;

/** `var/sw-ai-sdk` when the root has a `var/`, else `.sw-ai-sdk`. */
export function lockDir(root) {
  return isDir(join(root, 'var')) ? VAR_LOCK_DIR : LOCK_DIR;
}

export function lockFile(root) {
  return `${lockDir(root)}/harness.lock.json`;
}

/** Whether this root still needs the managed `.gitignore` line for the lock. */
export function lockNeedsGitignore(root) {
  return lockDir(root) === LOCK_DIR;
}
