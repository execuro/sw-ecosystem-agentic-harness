...
If you edited an agent or a skill, regenerate the host adapters first and
commit the result:
...
- **Every `SKILL.md` stays under 8192 bytes**, Codex's effective cap and the
  smallest of the four hosts'. A test enforces it. When a skill grows past the
  cap, move material into `reference/*.md` beside it — that is
...
- **Skills stay host-neutral.** No `${CLAUDE_SKILL_DIR}`, no `$ARGUMENTS`, no
