...
test('neither companion present — skipped everywhere, no host reads as drift', SKIP, () => {
  ...
  try {
    ...
    assert.equal(status.hosts.some((h) => h.state === 'drift' || h.state === 'not-installed'), false);
...
test('--host codex writes under .agents/skills, never .claude/skills', SKIP, () => {
  const { dir } = fakeNpx({ bodies: BODY });
  const root = hostRepo({ dirs: ['.codex', '.agents'] });
  try {
    run(['apply', '--yes', '--host', 'codex', '--root', root], { env: withNpx(dir) });
    assert.ok(exists(root, '.agents/skills/sw-specs-editor/SKILL.md'));
    assert.equal(exists(root, '.claude/skills/sw-specs-editor/SKILL.md'), false);
  } finally { cleanup(root); }
});

test('--host cursor writes under .cursor/skills alone', SKIP, () => {
  const { dir } = fakeNpx({ bodies: BODY });
  const root = hostRepo({ dirs: ['.cursor'] });
  try {
    run(['apply', '--yes', '--host', 'cursor', '--root', root], { env: withNpx(dir) });
